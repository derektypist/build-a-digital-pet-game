  ...
  return (
    <div className="stat-bar stat">
      ...
      <div className="stat-progress">
        <div className={`stat-fill ${colorClass}`} style={{ width: `${value}%` }}/>
      </div>
    </div>
  );
}
...
export const PetGame = () => {
  ...
  let { pet, doAction, setName} = usePet({isGameStarted, setGameStarted});

  const [fact, setFact] = useState<string | null>("");

  useEffect(() => {
    fetch("https://cat-facts-api.freecodecamp.rocks/api/catfacts/random").then(res => res.json()).then(fact => setFact(fact)).catch(err => {setFact("Sorry, we're not able to retrieve your cat fact right now");
  });
},[]);
    ...
    <main>
      ...
      {isGameStarted && (
        <section className="base-container game-container">
          ...
          <div className="pet-buttons">
            <button onClick={() => doAction(PetAction.EAT)} className="pet-button pet-buttons-left" id="eat-action">EAT</button>

            <button onClick={() => doAction(PetAction.PLAY)} className="pet-button pet-buttons-center" id="play-action">PLAY</button>

            <button onClick={() => doAction(PetAction.SLEEP)} className="pet-button pet-buttons-right" id="sleep-action">SLEEP</button>
          </div>
        </section>
      )}
      ...
      {isGameStarted && (
        <section className="stats-grid">
          <StatBar label="Hunger" value={pet.hunger} icon="🍔" reverse />
          <StatBar label="Happiness" value={pet.happiness} icon="😊" reverse />
          <StatBar label="Energy" value={pet.energy} icon="🔋" reverse />
        </section>
      )}
	...
	<section className="base-container info-panel">
        {!isGameStarted ? (
          <form className="start-questions">
            ...
            <input id="pet-name" name="pet-name" required={true} pattern="[A-Za-z0-9]{1,20}" />

            <button id="set-name-btn" onClick={startGame}>Start Game</button>
          </form>
        ) : (
          <div id="hud">
            <p id="pet-species">Species: {pet.species}</p>
            <p id="pet-fact"><b>Pet Fact:</b> {fact}</p>
          </div>
        )}
      </section>
    </main>
